...
import saborescerrado.jp.tp2.dto.PessoaFisicaResponseDTO;
...
import saborescerrado.jp.tp2.repository.PessoaFisicaRepository;
...
public class PessoaFisicaServiceImpl implements PessoaFisicaService {
    ...
    public Response getAll() {
        try {
            ...
            List<PessoaFisicaResponseDTO> pessoas = repository.findAll().stream()
            ...
            LOG.error("Erro ao rodar Requisição PessoaFisica.getAll()", e);
    ...
    public Response getAllAdmin(int page, int pageSize) {
        try {
            LOG.info("Requisição PessoaFisica.getAllAdmin()");
            List<PessoaFisicaResponseDTO> pessoas = repository.findAll().page(page, pageSize).stream()
                    .sorted(Comparator.comparing(EntityClass::getId).reversed())
                    .map(this::toResponseDTO)
                    .collect(Collectors.toList());
            return Response.ok(pessoas).build();
        } catch (Exception e) {
            LOG.error("Erro ao rodar Requisição PessoaFisica.getAllAdmin()", e);
            return Response.status(400).entity(e.getMessage()).build();
        }
    }
    ...
    public Response getId(long id) {
        try {
            ...
            LOG.error("Erro ao rodar Requisição PessoaFisica.getId()", e);
            return Response.status(400).entity(e.getMessage()).build();
        }
    }
    ...
    public Response getAdminId(long id) {
        try {
            LOG.info("Requisição PessoaFisica.getAdminId()");
            PessoaFisica pessoa = repository.findById(id);
            if (pessoa == null)
                return Response.status(Status.NOT_FOUND).build();
            return Response.ok(toResponseDTO(pessoa)).build();
        } catch (Exception e) {
            LOG.error("Erro ao rodar Requisição PessoaFisica.getAdminId()", e);
            return Response.status(400).entity(e.getMessage()).build();
        }
    }

    @Override
    public Response getCpf(String cpf) {
        try {
            LOG.info("Requisição PessoaFisica.getCpf()");
            PessoaFisica pessoa = repository.findByCpf(cpf);
            if (pessoa == null || !pessoa.getAtivo())
                return Response.status(Status.NOT_FOUND).build();
            return Response.ok(toResponseDTO(pessoa)).build();
        } catch (Exception e) {
            LOG.error("Erro ao rodar Requisição PessoaFisica.getCpf()", e);
            return Response.status(400).entity(e.getMessage()).build();
        }
    }
    ...
    public Response insert(PessoaFisicaDTO pessoaDTO) {
        try {
            ...
            LOG.error("Erro ao rodar Requisição PessoaFisica.insert()", e);
            return Response.status(400).entity(e.getMessage()).build();
        }
    }
    ...
    @Override
    @Transactional
    public Response update(long id, PessoaFisicaDTO pessoaDTO) {
        try {
            ...
            if (pessoaDTO.senha() != null && !pessoaDTO.senha().isEmpty())
                pessoa.setSenha(pessoaDTO.senha());
            if (pessoaDTO.cpf() != null && !pessoaDTO.cpf().isEmpty())
                pessoa.setCpf(pessoaDTO.cpf());
            ...
            LOG.error("Erro ao rodar Requisição PessoaFisica.update()", e);
            return Response.status(400).entity(e.getMessage()).build();
        }
    }
    ...
    public Response delete(Long id) {
        try {
            ...
            LOG.error("Erro ao rodar Requisição PessoaFisica.delete()", e);
            return Response.status(400).entity(e.getMessage()).build();
        }
    }

    private PessoaFisicaResponseDTO toResponseDTO(PessoaFisica pessoa) {
